...
export type VerifiedCrmUserContext = {
  authUserId: string;
  authEmail: string;
  crmUserId: string;
...
export type VerifiedAdminContext = VerifiedCrmUserContext;
...
export async function verifySignedInCrmUser(request: Request): Promise<
  | { context: VerifiedCrmUserContext; response: null }
  | { context: null; response: NextResponse }
> {
  const authorization = request.headers.get("authorization") || "";
  if (!authorization.startsWith("Bearer ")) {
    return { context: null, response: unauthorized("A signed-in Supabase session is required.") };
  }

  const accessToken = authorization.slice("Bearer ".length).trim();
  if (!accessToken) {
    return { context: null, response: unauthorized("The Supabase access token is missing.") };
  }
  ...
  if (authError || !authData.user) {
    return {
      context: null,
      response: unauthorized(authError?.message || "The Supabase session is invalid."),
    };
  }
  ...
  if (!authEmail) {
    return {
      context: null,
      response: forbidden("The signed-in Supabase account does not have an email address."),
    };
  }
  ...
  if (!crmUser) {
    return {
      context: null,
      response: forbidden("The signed-in account is not matched to a CRM Users record."),
    };
  }

  const crmStatus = String(crmUser.status || "").trim().toLowerCase();
  const crmRole = String(crmUser.user_role || "").trim().toLowerCase();

  if (crmStatus !== "active") {
    return {
      context: null,
      response: forbidden("The matched CRM Users record is not active."),
    };
  }

  if (!["admin", "sales_manager", "sales_rep"].includes(crmRole)) {
    return {
      context: null,
      response: forbidden("The matched CRM Users record does not have a supported CRM role."),
    };
  }
  ...
  return {
    context: {
      ...
      crmDisplayName: String(crmUser.display_name || authEmail),
      crmRole,
      crmStatus,
    },
    response: null,
